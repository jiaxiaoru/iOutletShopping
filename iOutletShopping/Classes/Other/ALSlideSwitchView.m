...
static const CGFloat kFontSizeOfTabButton = 17.0f; //button的字体大小
static const NSUInteger kTagOfRightSideButton = 999;


@interface ALSlideSwitchView()<UIScrollViewDelegate>
{
...
@implementation ALSlideSwitchView
#pragma mark - 初始化参数
...
- (void)buildUIFix:(BOOL)fix
{
    ...
    [self createNameButtonsWithFix:fix];
 ...
 * @result
 */
- (void)createNameButtonsWithFix:(BOOL)fix
{
    ...
    CGFloat btnWidth = (self.bounds.size.width-kWidthOfButtonMargin*(count+1))/count;
    for (int i = 0; i < count; i++) {
        UIViewController *vc = _viewArray[i];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        //如果不固定按钮的宽度，则根据文字来调整按钮的宽度
        if (!fix) {
            CGRect textRect = [vc.title boundingRectWithSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:kFontSizeOfTabButton]} context:nil];
            
            //累计每个tab文字的长度
            topScrollViewContentWidth += kWidthOfButtonMargin+textRect.size.width;
            //设置按钮尺寸
            btnWidth = textRect.size.width;

        }
        [button setFrame:CGRectMake(xOffset,0,
                                    btnWidth, kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
...
-(void)scrollHandlePan:(UIPanGestureRecognizer*) panParam
{
    ...
    if(_rootScrollView.contentOffset.x <= 0) {
        if (_delegate
            && [_delegate respondsToSelector:@selector(slideSwitchView:panLeftEdge:)]) {
